...
async function loginUser(prevState, formData) {
  ...
  // Check for missing fields or wrong credentials
  ...
  if (username !== 'admin' || password !== '1234') {
    ...
    return { error: 'Invalid credentials', errorCount: newErrorCount };
  }

  // Success: reset errorCount
  ...
  return (
    <form action={formAction}>
      <div>
        ...
        <input type="text" name="username" disabled={isLocked} />
      </div>
      <div>
        <label htmlFor="password">Enter password </label>
        <input type="password" name="password" disabled={isLocked} />
      </div>
      ...
      {state.errorCount > 0 && state.errorCount < 3 && (
        <div style={{ color: 'orange' }}>
          Failed attempts: {state.errorCount} / 3
        </div>
      )}
    </form>
  );
